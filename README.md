...
* <a href="#input-files">2. Input files</a>
* <a href="running-the-simulations">3. Running the simulations</a>
...
## Input files
<p align="justify">
The program used for MD simulations was GROMACS (5.0.2).
</p>
...
First, the equilibration .tpr is configured using the grompp command:

```console
gmx grompp -f nvt_equilibration.mdp -c calcite_met_eth_35A.gro -p topol.top -o nvt_eq.tpr

```
For further details on how to use gmx grompp the user is referred to the GROMACS <a href="https://manual.gromacs.org/documentation/5.1/onlinehelp/gmx-grompp.html">documentation</a> for the topic.

With the .tpr it is possible to run the equilibration.
...
Simular procedures are also valid to perform the production simulation. However, it is necessary to use the output from the equilibration as inputs.
</p>

```console
gmx grompp -f nvt_production.mdp -c nvt_eq.gro -p topol.top -o nvt_prod.tpr

gmx mdrun -deffnm nvt_prod
